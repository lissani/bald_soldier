...
float hat_angle = 0.0f;
float hat_radius = 30.0f;
float hat_angle_speed = 1.0f;
glm::vec2 hat_center = glm::vec2(0.0f, 0.0f);
glm::vec2 hat_offset = glm::vec2(0.0f, 0.0f);
bool hat_flying = true;

void reset_hat_flight() {
	float rand_radius = 20.0f + (rand() % 30);
	float rand_angle = (rand() % 360) * TO_RADIAN;
	hat_angle = 0.0f;
	hat_radius = rand_radius;
	hat_angle_speed = 1.0f + (rand()%100)/100.f;
	hat_center = glm::vec2(0.0f, 0.0f); // 출발 지점
	hat_offset = glm::vec2(cos(rand_angle), sin(rand_angle));
}
